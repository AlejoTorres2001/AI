...
def cluster_embeddings(embeddings:list[list[float]],num_clusters:int):
  ...
  return k_means
def get_documents_by_index(indices:list[int],docs:list[Document]):
  selected_docs = [docs[doc] for doc in indices]
...
# Loop through the number of clusters you have
  for i in range(num_clusters):
    ...
    distances = np.linalg.norm(embeddings - k_means.cluster_centers_[i], axis=1)
